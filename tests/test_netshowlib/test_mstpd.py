...
def test_cacheinfo(mock_exec):
    ...
    assert_equals(sorted(_output.keys()), ['br0', 'br2'])
    assert_equals(sorted(_output.get('br0').get('ifaces').keys()),
                  ['swp3', 'swp4'])
    assert_equals(_output.get('br0').get('ifaces').get('swp3').get('auto_edge_port'),
                  'yes')
    assert_equals(_output.get('br0').get('designated_root'),
